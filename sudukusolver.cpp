...
bool Sudoku::solveSudoku(cell t_board[9][9])
{
   ...
   { //valid numbers are 1 - 9
      ...
      { //check validation, if yes, put the number in the grid
         ...
         if (solveSudoku(t_board)) //recursively go for other rooms in the grid
            return true;
         t_board[row][col].set_Val(-1,true); //turn to unassigned space when conditions are not satisfied
      }
   }
   return false;
}
